export async function waitForElementToVisible(page, selector, label) {
    await page.locator(selector).isVisible();
}
export async function waitForElementToEnabled(page, selector, label) {
    await page.locator(selector).isEnabled();
}
export async function waitForElementToVisibleWithRetry(page, selector, retries = 3, timeout = 25000, delayBetweenRetries = 1000, label = 'element', elemenState = String) {
