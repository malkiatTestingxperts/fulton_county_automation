...
exports.configureHealing = configureHealing;
exports.heuristicScore = heuristicScore;
exports.generateLocatorCandidates = generateLocatorCandidates;
const SynonymService_1 = require("./SynonymService");
...
function configureHealing(config) {
...
function isVolatileClass(cls) {
    return (cls.length <= 12 &&
        /(active|loading|disabled|selected|focus|error|open|show)/i.test(cls));
}
// 🔹 Scoring function
function heuristicScore(candidate, meta) {
    const sel = candidate.selector;
    const attrs = meta.attributes || {};
    if (sel.includes("#") || sel.includes("aria-label") || /@id=/.test(sel)) {
        const id = attrs["id"] || "";
        if (id && /^[a-zA-Z]+[a-zA-Z0-9_-]*$/.test(id) && id.length < 15)
            return 95;
        return 80;
    }
    if (sel.includes("normalize-space(text())"))
        return 90;
    if (sel.includes("contains(normalize-space"))
        return 75;
    if (sel.includes("[") && sel.includes("="))
        return 70;
    if (sel.includes(" and @"))
        return 80;
    if (sel.includes("starts-with(") || sel.includes(" or @"))
        return 65;
    if (candidate.type.includes("class"))
        return 70;
    if (sel.includes("nth-child") || sel.includes("sibling") || sel.includes("ancestor") || sel.includes("descendant"))
        return 50;
    if (candidate.type.includes("synonym"))
        return 35;
    if (candidate.type.includes("absolute-xpath"))
        return 10;
    return 50;
}
// --- Validate uniqueness
async function validateUnique(page, selector) {
    try {
...
async function addIfUnique(page, candidates, type, selector) {
...
async function generateLocatorCandidates(page, originalSelector) {
    const candidates = [];
    ...
    if (!elementHandle) {
        ...
        if (parentMatch)
            expectedParentId = parentMatch[2];
        ...
        for (const el of all) {
            ...
            if (attrs["type"] === "submit")
                simScore += 0.3;
            // Parent check
            const parentId = await el.evaluate((node) => node.parentElement?.id || "");
            const parentClass = await el.evaluate((node) => node.parentElement?.className || "");
            if (expectedParentId && parentId === expectedParentId)
                simScore += 0.4;
            if (expectedParentClass && parentClass.includes(expectedParentClass))
                simScore += 0.3;
    ...
    const meta = await page.evaluate((el) => {
        if (!el)
            return { tag: "", text: "", attributes: {}, ariaLabel: "", role: "", parent: { id: "", class: "", tag: "" } };
        const attrs = {};
    ...
    if (text && text.length < 50) {
        ...
        const synonyms = await (0, SynonymService_1.getSynonyms)(text);
        const seen = new Set();
    ...
    // --- Absolute XPath
    const absoluteXPath = await page.evaluate((el) => {
        if (!el)
            return "";
        const getPath = (node) => {
